/**
 * File: time_complexity.js
 * Created Time: 2023-01-02
 * Author: RiverTwilight (contact@rene.wang)
 */

/* Konstans rend */
function constant(n) {
    let count = 0;
    const size = 100000;
    for (let i = 0; i < size; i++) count++;
    return count;
}

/* Lineáris rend */
function linear(n) {
    let count = 0;
    for (let i = 0; i < n; i++) count++;
    return count;
}

/* Lineáris rend (tömb bejárása) */
function arrayTraversal(nums) {
    let count = 0;
    // Az iterációk száma arányos a tömb hosszával
    for (let i = 0; i < nums.length; i++) {
        count++;
    }
    return count;
}

/* Exponenciális rend */
function quadratic(n) {
    let count = 0;
    // Az iterációk száma négyzetes kapcsolatban áll az n adatmérettel
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < n; j++) {
            count++;
        }
    }
    return count;
}

/* Négyzetes rend (buborékrendezés) */
function bubbleSort(nums) {
    let count = 0; // Számláló
    // Külső ciklus: a rendezetlen tartomány [0, i]
    for (let i = nums.length - 1; i > 0; i--) {
        // Belső ciklus: a [0, i] rendezetlen tartomány legnagyobb elemét a tartomány jobb végére cseréljük
        for (let j = 0; j < i; j++) {
            if (nums[j] > nums[j + 1]) {
                // Felcseréljük a nums[j] és nums[j + 1] elemeket
                let tmp = nums[j];
                nums[j] = nums[j + 1];
                nums[j + 1] = tmp;
                count += 3; // Egy elemcsere 3 elemi műveletet tartalmaz
            }
        }
    }
    return count;
}

/* Exponenciális rend (ciklus implementáció) */
function exponential(n) {
    let count = 0,
        base = 1;
    // A sejtek minden körben kettéosztódnak, 1, 2, 4, 8, ..., 2^(n-1) sorozatot alkotva
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < base; j++) {
            count++;
        }
        base *= 2;
    }
    // count = 1 + 2 + 4 + 8 + .. + 2^(n-1) = 2^n - 1
    return count;
}

/* Exponenciális rend (rekurzív implementáció) */
function expRecur(n) {
    if (n === 1) return 1;
    return expRecur(n - 1) + expRecur(n - 1) + 1;
}

/* Logaritmikus rend (ciklus implementáció) */
function logarithmic(n) {
    let count = 0;
    while (n > 1) {
        n = n / 2;
        count++;
    }
    return count;
}

/* Logaritmikus rend (rekurzív implementáció) */
function logRecur(n) {
    if (n <= 1) return 0;
    return logRecur(n / 2) + 1;
}

/* Lineáris-logaritmikus rend */
function linearLogRecur(n) {
    if (n <= 1) return 1;
    let count = linearLogRecur(n / 2) + linearLogRecur(n / 2);
    for (let i = 0; i < n; i++) {
        count++;
    }
    return count;
}

/* Faktoriális rend (rekurzív implementáció) */
function factorialRecur(n) {
    if (n === 0) return 1;
    let count = 0;
    // 1-ből n-re osztódik
    for (let i = 0; i < n; i++) {
        count += factorialRecur(n - 1);
    }
    return count;
}

/* Tesztkód */
// Az n értékét módosíthatja a futtatáshoz és megfigyelheti a különböző bonyolultságú műveletek számának trendjét
const n = 8;
console.log('Input data size n = ' + n);

let count = constant(n);
console.log('Constant order operation count = ' + count);

count = linear(n);
console.log('Linear order operation count = ' + count);
count = arrayTraversal(new Array(n));
console.log('Linear order (array traversal) operation count = ' + count);

count = quadratic(n);
console.log('Quadratic order operation count = ' + count);
let nums = new Array(n);
for (let i = 0; i < n; i++) nums[i] = n - i; // [n,n-1,...,2,1]
count = bubbleSort(nums);
console.log('Quadratic order (bubble sort) operation count = ' + count);

count = exponential(n);
console.log('Exponential order (loop implementation) operation count = ' + count);
count = expRecur(n);
console.log('Exponential order (recursive implementation) operation count = ' + count);

count = logarithmic(n);
console.log('Logarithmic order (loop implementation) operation count = ' + count);
count = logRecur(n);
console.log('Logarithmic order (recursive implementation) operation count = ' + count);

count = linearLogRecur(n);
console.log('Linearithmic order (recursive implementation) operation count = ' + count);

count = factorialRecur(n);
console.log('Factorial order (recursive implementation) operation count = ' + count);
